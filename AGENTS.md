...
产品入口是根目录的 `MarkPivot.html`。这是一个本地优先的原生 JS 单文件工具。面向用户的主流程是文档规范化（上传 Word → 选择规范 → 微调 → 导出）；Markdown 仍是内部语义层，不要做 Excel↔Word 直转入口。
...
M0 + M1 + M2 + M3 + M4 + M5 + M6 + M7 + M8 + M9 + M10 + M11 已落地。规范库后续（版本 / 标签 / 搜索 / 最近使用）尚未做。
