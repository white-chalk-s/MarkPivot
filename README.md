...
2026-09-24 完成 **M0–M15**：入口统一为 `MarkPivot.html`，主流程为独立任务「上传文件 → 选择规范 → 自动识别/微调 → 批量导出」。规范库可新建、编辑、复制、导入导出 JSON、从 Word 创建；任务内微调不会自动覆盖库里的规范。工作台采用暖白橙三栏布局；左侧文件栏下方显示当前文档标题大纲，支持点击定位和滚动高亮，并在确认待识别标题后更新。AI 识别窗口可关闭，宽屏打开时预览自动避让并适配纸面缩放，关闭后恢复原缩放，窄屏作为浮层显示。工作台中栏可切换原文/当前效果；完成调整后进入导出页。首期 AI 格式匹配接入 DeepSeek，用户主动点击扫描后才发送当前文章和格式要求；API Key 默认仅保存在当前浏览器会话，扫描建议需确认后应用。规范栏保留页面、正文、标题、表格、图片全部 Tab 和字段，格式参数改为常用选项；模板现有特殊值以“保留当前值”选项展示，不提供自由输入。解压包需保持 `MarkPivot.html`、`vendor/`、`templates/` 同层；不使用 AI 时带本地运行库可离线启动。
...
带着 `vendor/` 首次打开不必访问 CDN。顶部状态显示「本地运行库可用（可离线）」时，9 个转换库都来自本机。只有缺少 vendor 且本机没有缓存时，才会尝试 jsDelivr / unpkg。AI 是可选网络功能；使用时需自行配置 DeepSeek API Key，文章内容会发送至 DeepSeek。`templates/reference-template.docx` 可作为参考规范样例上传。
...
- DeepSeek 格式匹配仅在用户主动调用时联网发送文章；不生成或改写正文。API Key 默认只保存在当前浏览器会话。
